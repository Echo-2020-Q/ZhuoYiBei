...
# >>> add: 地理围栏配置
GEOFENCE_MARGIN_M   = 1500.0   # 距离边界的安全缓冲（米）；小于该距离将纠偏
SPEED_NEAR_BOUNDARY = 180.0    # 贴边时的限速（m/s）
HEADING_CANDIDATES  = [0, 15, -15, 30, -30, 60, -60, 90, -90, 120, -120, 150, -150, 180]  # 尝试修正角

# 禁飞圆区：[(lon, lat, radius_m), ...]
NO_FLY_CIRCLES = [
    # 例：自定义一个禁飞圆
    # (102.20, 40.10, 6000.0),
]
...
# >>> add: Formation params
FORMATION_SPACING_M = 1200.0     # 机间水平间距（米）
FORMATION_SPEED_MPS  = 240.0     # 编队前进水平速度
FORMATION_VZ_KEEP    = 0.0       # 编队时的垂直速度（保持高度）
FORMATION_TIMEOUT_S  = 120.0     # 编队阶段最长墙钟时间（秒）
SECTOR_NUM           = 12        # 360°划分 N 个扇区，用于“未探索”选择
SECTOR_MARK_RADIUS_M = 8000.0    # 视为“已探索”的半径（编队途经+雷达扫描）
UNKNOWN_BONUS_DEG    = 40.0      # 优先指向“离边界更远”的扇区时的角度偏好（可选）
# --- Formation tightening (队形收敛/保持) ---
FORMATION_TIGHT = True          # 开启“收敛保持”
SLOT_GAIN = 0.6                 # 槽位误差 → 水平速度加成 (m/s per meter) 的比例（会被限幅）
SLOT_MAX_BOOST = 80.0           # 槽位误差速度加成上限 (m/s)
SLOT_TOL_M = 80.0               # 认为已到位的误差阈值（米），小于它就不再加速追
LEADER_ID_FIXED = 10085          # 若想固定某个 rid 做队长，填入数值；None 表示默认列表第一个


# ==================== 通用工具函数 ====================
# >>> add: 小工具
# --- 经纬与局部米坐标互换（以参考点 lat0 为准） ---

def _rotate_offsets_body_to_NE(offset_body, heading_deg):
    """
    把机体系(前=北轴)下的 (ΔN, ΔE) 偏置，旋转到“世界 NE 坐标系”，
    其中 heading_deg = 编队朝向（0=北，顺时针）。
    返回 (ΔN_world, ΔE_world)
    """
    import math
    th = math.radians(heading_deg)
    c, s = math.cos(th), math.sin(th)
    dN_b, dE_b = offset_body
    # 机体系前(北)→世界 N；右(东)→世界 E；顺时针为正
    dN_w = dN_b * c + dE_b * s
    dE_w = -dN_b * s + dE_b * c
    return (dN_w, dE_w)
def _lonlat_to_local_m(lon, lat, lon0, lat0):
    """
    返回 (north_m, east_m)，以 (lon0,lat0) 为原点的近似米制坐标。
    """
    import math
    if None in (lon, lat, lon0, lat0):
        return (None, None)
    d_n = (lat - lat0) * 110540.0
    d_e = (lon - lon0) * 111320.0 * math.cos(math.radians(lat0))
    return (d_n, d_e)
# >>> add: 围栏/禁飞区工具
def _point_in_circle(lon, lat, c_lon, c_lat, r_m):
    d = _geo_dist_haversine_m(lon, lat, c_lon, c_lat) or 1e18
    return d <= float(r_m)

def _nearest_rect_center():
    # 矩形中心（回中心偏航用）
    cx = 0.5*(BOUNDARY_RECT["min_x"] + BOUNDARY_RECT["max_x"])
    cy = 0.5*(BOUNDARY_RECT["min_y"] + BOUNDARY_RECT["max_y"])
    return cx, cy

def _inside_boundary(lon, lat, margin_m=0.0):
    dL, dR, dD, dU = _dist_to_boundary_m(None, lon, lat)  # 这个函数内部已处理client缺失
    return (min(dL, dR, dD, dU) - 1e-6) >= float(margin_m)

def _too_close_to_boundary(lon, lat, margin_m):
    dL, dR, dD, dU = _dist_to_boundary_m(None, lon, lat)
    return min(dL, dR, dD, dU) < float(margin_m)

def _would_violate_next(lon, lat, heading_deg, spd_mps, dt_s, circles):
    """
    预测dt后位置是否触犯围栏或禁飞区（只看一步/短视窗）
    """
    # 用局部米制近似前推
    n,e = _lonlat_to_local_m(lon, lat, lon, lat)  # 原点
    import math
    dn = spd_mps * dt_s * math.cos(math.radians(heading_deg))  # 朝北为0°
    de = spd_mps * dt_s * math.sin(math.radians(heading_deg))  # 朝东为正
    # 转回经纬
    lon2, lat2 = _local_m_to_lonlat(dn, de, lon, lat)
    if not _inside_boundary(lon2, lat2, margin_m=GEOFENCE_MARGIN_M):
        return True
    for (clon, clat, cr) in circles:
        if _point_in_circle(lon2, lat2, clon, clat, cr + 0.0):  # 禁飞区边界不碰
            return True
    return False

def _aggregate_no_fly_circles_cached(client):
    """
    组合禁飞圆区（含可选干扰圈），并做一次半径求值缓存。
    """
    circs = list(NO_FLY_CIRCLES) if NO_FLY_CIRCLES else []
    if NO_FLY_USE_JAM:
        global JAM_RADIUS_M
        # 求干扰圈半径（若未算）
        ...
        circs.append((JAM_CENTER[0], JAM_CENTER[1], float(JAM_RADIUS_M)))
    return circs

def _guard_heading(client, lon, lat, heading_deg, spd_mps, prefer_center=True):
    """
    航向守卫：若当前航向会导致“贴边/越界/闯禁飞区”，尝试从预设角度集合中找到最近的安全航向。
    规则：
      - 优先保持原角；若不安全，尝试 ±15/±30/... 等候选；
      - 若都不安全，朝矩形中心方向；若仍不安全，最后180°掉头+限速。
    返回 (safe_heading_deg, safe_speed_mps)
    """
    circles = _aggregate_no_fly_circles_cached(client)

    # 1) 若当前位置已越界（理论上不应发生），先指向矩形中心
    if not _inside_boundary(lon, lat, margin_m=0.0):
        cx, cy = _nearest_rect_center()
        to_center = _bearing_deg_from_A_to_B(lon, lat, cx, cy) or 0.0
        return to_center, min(spd_mps, SPEED_NEAR_BOUNDARY)

    # 2) 贴边限速
    near = _too_close_to_boundary(lon, lat, GEOFENCE_MARGIN_M) or any(
        _point_in_circle(lon, lat, clon, clat, cr + GEOFENCE_MARGIN_M*0.2) for clon,clat,cr in circles
    )
    spd_try = min(spd_mps, SPEED_NEAR_BOUNDARY) if near else spd_mps

    # 3) 原角可行？
    if not _would_violate_next(lon, lat, heading_deg, spd_try, dt_s=2.0, circles=circles):
        return heading_deg % 360.0, spd_try

    # 4) 尝试小幅修正
    for d in HEADING_CANDIDATES:
        h2 = (heading_deg + d) % 360.0
        if not _would_violate_next(lon, lat, h2, spd_try, dt_s=2.0, circles=circles):
            return h2, spd_try
    ...
    if prefer_center:
        cx, cy = _nearest_rect_center()
        to_center = _bearing_deg_from_A_to_B(lon, lat, cx, cy) or 0.0
        if not _would_violate_next(lon, lat, to_center, SPEED_NEAR_BOUNDARY, dt_s=2.0, circles=circles):
            return to_center, SPEED_NEAR_BOUNDARY

    # 6) 仍失败：180°掉头 + 更低限速
    return (heading_deg + 180.0) % 360.0, min(spd_try, 120.0)

def _local_m_to_lonlat(n_m, e_m, lon0, lat0):
    """
    可选：若有需要把米坐标回转成经纬（本补丁里不必用到）
    """
    import math
    if None in (n_m, e_m, lon0, lat0):
        return (None, None)
    lat = lat0 + (n_m / 110540.0)
    lon = lon0 + (e_m / (111320.0 * math.cos(math.radians(lat0))))
    return (lon, lat)

def _deg_to_rad(d):
    import math; return math.radians(float(d))

def _rot_xy(x, y, heading_deg):
    """以 0°=北，顺时针 的坐标系旋转 (机体朝向系)。返回旋转后的 (x', y')."""
    import math
    th = _deg_to_rad(heading_deg)
    # 我们的约定：x轴指北，y轴指东；顺时针转为数学坐标需注意正负
    # 采用“北为0°, 顺时针为正”的直接旋转：
    c, s = math.cos(th), math.sin(th)
    xn =  x*c + y*s
    yn = -x*s + y*c
...
def _sector_idx_from_heading(h, n=SECTOR_NUM):
    """0..n-1"""
    h = _ang_norm(h)
    w = 360.0 / n
    return int(h // w) % n

def _sector_center_deg(i, n=SECTOR_NUM):
    w = 360.0 / n
    return (i + 0.5) * w % 360.0
...
# >>> Formation Manager编队飞行
class FormationManager:
    """
    负责 V 形编队与“未探索扇区”选择。
    """
    def __init__(self, red_ids):
        self.red_ids = sorted(int(x) for x in red_ids)
        assert len(self.red_ids) == 4, "目前按 4 架红机设计 V 形编队"
        self.leader_id = self.red_ids[0]  # 以列表第一个为队长（可按需换）
        self.start_wall = time.time()
        # 扇区最近“探索时间”，初始化为 -inf（从未探索）
        self.sector_last_seen = [-1e18] * SECTOR_NUM
        # 固定 V 形相对队长偏置（单位：米），在“机体朝向=0° 北”的机体系下
        d = FORMATION_SPACING_M
        self.rel_offsets_body = {
            self.red_ids[0]: (0.0, 0.0),        # 队长
            self.red_ids[1]: (-d, -0.7*d),      # 左后
            self.red_ids[2]: (-d,  0.7*d),      # 右后
            self.red_ids[3]: (-2*d, 0.0),       # 尾机
        }
        if LEADER_ID_FIXED in self.red_ids:
            self.leader_id = int(LEADER_ID_FIXED)



    def mark_explored_by_path(self, client, pos_all):
        """
        编队所在位置附近扇区打点（近似代表‘已探索/已访问’）。
        """
        tnow = time.time()
        # 用队长位置和航向更新扇区
    ...
    def mark_explored_by_radar(self, red_id, heading_deg=None):
        """
        若雷达有扫描（可见表非空）时，以当前指向扇区作‘已探索’。
        """
        if heading_deg is None:
            return
        idx = _sector_idx_from_heading(heading_deg, SECTOR_NUM)
        self.sector_last_seen[idx] = max(self.sector_last_seen[idx], time.time())

    def choose_unexplored_heading(self, client, pos_all):
        """
        选‘最久未探索’的扇区中心角作为期望前进方向。
        额外启发：离边界更远的扇区轻微加权（UNKNOWN_BONUS_DEG）。
        """
        # 基于“队长→四边的可行空间”给一点边界启发
        pL = pos_all.get(self.leader_id)
        ...
        # 选 last_seen 最小者
        oldest = min(self.sector_last_seen)
        cand_idxs = [i for i, t in enumerate(self.sector_last_seen) if abs(t - oldest) < 1e-6]
        # 多候选时，选更靠近 wide_dir 的扇区
        def score(i):
...
class RedForceController:
    def __init__(self, client, red_ids, out_csv_path):
        ...
        self.fm = FormationManager(self.red_ids)
        # --- 调试/打印控制导弹的轨迹 ---
    ...
    def step(self):
        ...
        if self.formation_active:
            # 条件：若有蓝机目标可见，则退出编队
            any_blue_visible = False
            for tracks in (vis or {}).values():
                if any(int(t.get("target_id", 0)) >= 10000 for t in (tracks or [])):
                    any_blue_visible = True
                    break

            # 超时保护（墙钟）
            if (time.time() - getattr(self.fm, "start_wall", time.time())) > FORMATION_TIMEOUT_S:
                any_blue_visible = True  # 到时退出编队

            if any_blue_visible:
                self.formation_active = False
            else:
                # --- 选择前进方向（最久未探索扇区） ---
                self.fm.mark_explored_by_path(self.client, all_pos)
                ...
                if pL and getattr(pL, "x", None) is not None and getattr(pL, "y", None) is not None:
                    ...
                    # 队长：直接朝 heading 以编队速度前进
                    try:
                        vcmd = rflysim.Vel()
                        safe_h, safe_spd = _guard_heading(self.client, LON0, LAT0, float(heading % 360.0),
                                                          float(FORMATION_SPEED_MPS))
                        vcmd.rate = safe_spd
                        vcmd.direct = safe_h
                    ...
                    for rid in sorted(self.red_ids):
                        ...
                        # 当前红机在“以队长为原点”的局部米系中的位置
                        n_me, e_me = _lonlat_to_local_m(my_lon, my_lat, LON0, LAT0)

                        # 该红机的槽位（机体系偏置 -> 世界 NE 偏置）
                        dN_body, dE_body = self.fm.rel_offsets_body.get(rid, (0.0, 0.0))
                        ...
                        try:
                            vcmd = rflysim.Vel()
                            safe_h, safe_spd = _guard_heading(self.client, my_lon, my_lat, float(use_heading),
                                                              float(speed_cmd))
                            vcmd.rate = safe_spd
                            vcmd.direct = safe_h
                            vcmd.vz = float(FORMATION_VZ_KEEP)
                            ...
                            print(f"[Formation] follower set_vehicle_vel({rid}) failed:", e, flush=True)

                    print(
                        f"[Formation] heading={heading:.1f}° speed={FORMATION_SPEED_MPS} m/s (tight={FORMATION_TIGHT})",
                        flush=True)

                # 仍然保持：编队阶段 return，下一 tick 继续；发现蓝机或超时则 self.formation_active=False
                return
