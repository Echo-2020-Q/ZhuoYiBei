# -*- coding: utf-8 -*-
"""
PPO(序列)版：批量采集 obs64+act16，并按“新攻击逻辑”发射。
- 与 PPOAgent.act(obs64, tfeat, h, explore) 对接，维护 RNN 隐状态与时间特征
- 记录 runs/<timestamp>/ep_XXXX.csv & manifest.json
- 保留：导弹规避、冷却/锁定/弹药、打击确认、调试打印
"""

import os, re, csv, json, time, math, random, threading
...
def _direct_rate_from_vx_vy(vx, vy):
    try:
        ...
        vxn, vye = 0.0, 0.0
    direct = (vxn*vxn + vye*vye) ** 0.5
    rate = (math.degrees(math.atan2(vye, vxn)) + 360.0) % 360.0
    return direct, rate
...
def _bearing_deg_A_to_B(lonA, latA, lonB, latB): return _bearing_deg_from_A_to_B(lonA, latA, lonB, latB)

def _is_missile_track(tid):
    try: return int(tid) < 10000
...
def _dist_to_boundary_m(client, lon, lat):
    if lon is None or lat is None: return (0.0, 0.0, 0.0, 0.0)
...
def _signed_dist_to_jam_boundary_m(client, lon, lat):
    ...
    if JAM_RADIUS_M is None:
        ...
        try: JAM_RADIUS_M = client.get_distance_by_lon_lat(C, E)
        except Exception: JAM_RADIUS_M = 10416.0
    P = Position(x=lon, y=lat, z=0)
    try: d_c = client.get_distance_by_lon_lat(C, P)
    except Exception: d_c = 0.0
...
def normalize_visible(visible):
    ...
    for detector_id, v in visible.items():
        ...
        if v is None:
            ...
            for t in v:
                item = _extract_track_fields_any(t) if (isinstance(t, dict) or hasattr(t, '__dict__')) else _parse_track_from_string_rich(t)
...
class RLRecorder:
    ...
    def pack_single_obs16(self, client, rid, obs, boundary_rect, jam_signed_dist, blue4_dists,
        ...
        if len(obs16) == 15:
            speed_scalar = (float(vx or 0.0) ** 2 + float(vy or 0.0) ** 2) ** 0.5
    ...
    def dump_csv(self, path):
        ...
        with open(abspath, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(header); w.writerows(self.rows_for_csv)
...
class RedForceController:
    def __init__(self, client, red_ids, out_csv_path):
        ...
        # ===== PPO 接入（你的接口）=====
        self._ppo = None                 # PPOAgent
        self._ppo_h = None               # RNN 隐状态
        self._ppo_t0 = None              # 时间基准
        self._ppo_tspan = 300.0          # 归一化时间尺度
        self._ppo_tdim = 3               # 默认；attach 后以 agent.tdim 为准
        self._ppo_explore = True         # 采样时默认探索
        self._policy_attack_intent = {}  # rid -> 0/1
    ...
    # == PPO 对接 ==
    def attach_ppo_policy(self, ppo_agent, explore=True, tspan_hint=300.0):
        """
        ppo_agent: 你的 PPOAgent 实例（含 .act(obs64, tfeat, h, explore) 和 .tdim）
        explore: 采样时是否启用随机性
        tspan_hint: 时间特征归一化尺度
        """
        self._ppo = ppo_agent
        self._ppo_h = None
        self._ppo_t0 = None
        self._ppo_tspan = float(tspan_hint) if tspan_hint and tspan_hint > 0 else 300.0
        self._ppo_tdim = int(getattr(ppo_agent, "tdim", 3) or 3)
        self._ppo_explore = bool(explore)
        print(f"[PPO] attached. tdim={self._ppo_tdim}, explore={self._ppo_explore}, tspan={self._ppo_tspan}", flush=True)

    def _time_feat(self, t: float):
        if self._ppo_t0 is None:
            self._ppo_t0 = float(t)
        t_norm = float(t - self._ppo_t0) / max(1.0, self._ppo_tspan)
        feats = [t_norm, math.sin(2*math.pi*t_norm), math.cos(2*math.pi*t_norm)]
        return np.asarray(feats[:self._ppo_tdim], dtype=np.float32)

    # ---- 辅助 ----
    def _is_blue_side(self, side): return side in (2, "SIDE_BLUE", "BLUE")
    def _is_destroyed_flag(self, dmg):
        if isinstance(dmg, (int, float)): return int(dmg) == 1
        s = str(dmg); return ("DESTROYED" in s.upper()) or (s.strip() == "1")
    ...
    def _update_destroyed_from_situ(self, force=False):
        ...
        if (len(self.destroyed_blue) >= 4 or len(self.destroyed_red) >= 4) and self._end_grace_until is None:
            ...
            print(f"[Grace] {self._end_reason} Ending in 20s...", flush=True); return
        if self._end_grace_until is not None and now_wall >= self._end_grace_until:
    ...
    def step(self):
        ...
        # === 每秒一次：PPO.act -> 16维动作，按新逻辑下发速度 + 记录攻击意图 ===
        try:
            if sim_sec != self._last_policy_sec and self.recorder.latest_obs_vec is not None:
                if self._ppo is not None:
                    obs64 = np.asarray(self.recorder.latest_obs_vec, dtype=np.float32)
                    if obs64.shape[0] != 64:
                        raise ValueError(f"obs length expected 64, got {obs64.shape[0]}")
                    tfeat = self._time_feat(sim_sec)
                    a_full, V, logp, h1 = self._ppo.act(obs64, tfeat, h=self._ppo_h, explore=self._ppo_explore)
                    self._ppo_h = h1
                else:
                    a_full = None
                ...
                if a_full is None:
                    ...
                    arr = list(a_full)
                    if len(arr) != 16:
                        raise ValueError(f"PPO act() must return 16 floats, got {len(arr)}")
                    red_list = sorted(self.red_ids)
                    for idx, rid in enumerate(red_list):
                        a0, a1, a2, a3 = [float(arr[idx*4 + k]) for k in range(4)]
                        vcmd = rflysim.Vel()
                        # 映射：策略 rate(航向角) -> 接口 direct；策略 direct(速度标量) -> 接口 rate
                        vcmd.rate   = max(0.0, float(a1))     # 速度
                        vcmd.direct = float(a0) % 360.0       # 航向
                        vcmd.vz     = float(a2)
        ...
        # ====== 之后与“新攻击逻辑”一致：导弹规避调试 + 触发规避 + 目标搜集 + 策略优先发射 + 回退分配 ======
        # -- 获取可见 & 位置 --
        ...
        for tracks in vis2.values():
            for t in tracks:
                ...
                missiles.append({"mid": mid, "lon": mlon, "lat": mlat, "mdir": mdir_calc, "mspeed": mspeed_calc,
        ...
        if missiles:
            ...
            for rid in sorted(self.red_ids):
                ...
                approach_ok, reason = False, "unknown"
                if ang_diff is not None:
                    if ang_diff <= self.MISSILE_BEARING_THRESH_DEG:
                        approach_ok, reason = True, f"angle_ok({ang_diff:.1f}<= {self.MISSILE_BEARING_THRESH_DEG})"
                    else:
                    ...
                    if last_d is not None and best_dist < last_d:
                        approach_ok, reason = True, "dist_decreasing"
                    else:
                ...
                if self.DEBUG_EVADE:
                    ...
                    if (time.time() - lp) >= self.MISSILE_DEBUG_PRINT_INTERVAL or flip:
                        ...
                        print(
                            f"[MISSILE] rid={rid} mid={best_mid} dist={dist_str}m "
                            f"mspeed_calc={mspeed_calc_str}m/s mspeed_radar={mspeed_radar_str}m/s "
                ...
                if in_threat and approach_ok and cooldown_ok:
                    ...
                    v_direct_now, _ = _direct_rate_from_vx_vy(getattr(v_me, "vx", 0.0) if v_me else 0.0,
                    ...
                    try:
                        ...
                        uid = self.client.set_vehicle_vel(rid, vcmd)
                        self.recorder.mark_vel_cmd(rid, rate=vcmd.rate, direct=vcmd.direct, vz=vcmd.vz, sim_time=now_sim)
                        print(f"[EVADE] rid={rid} mid={best_mid} CMD: rate={vcmd.rate:.1f} direct={vcmd.direct:.1f} vz={vcmd.vz:.1f}", flush=True)
                    except Exception as e:
                        print(f"[EVADE] set_vehicle_vel({rid}) failed: {e}", flush=True)
                    self._evasive_until[rid] = now_sim + self.EVASIVE_DURATION_SEC
                    self._last_evasive_time[rid] = time.time()
        ...
        if not assignments and not any(self._policy_attack_intent.get(r, 0) for r in self.red_ids):
            return
        ...
        try:
            ...
            for rid in red_list:
                if not self._policy_attack_intent.get(rid, 0): continue
...
def run_one_episode(client, plan_id, out_csv_path, ppo_agent=None,
                    max_wall_time_sec=360, min_wall_time_sec=10, explore=True):
    try:
    ...
    blue_thread = threading.Thread(target=_blue_wrapper, daemon=True); blue_thread.start()
    print("[Runner] Blue loop started.", flush=True)

    # 红控（接 PPO）
    red_ctrl = RedForceController(client, RED_IDS, out_csv_path)
    if ppo_agent is not None:
        red_ctrl.attach_ppo_policy(ppo_agent, explore=explore, tspan_hint=300.0)

    red_thread = threading.Thread(target=lambda: red_ctrl.run_loop(max_wall_time_sec=max_wall_time_sec),
                                  daemon=True)
    red_thread.start()
    print("[Runner] Red loop started.", flush=True)

    t0 = time.time(); success = False; score_obj = None
    try:
        while True:
        ...
        try: score_obj = red_ctrl._fetch_score_with_retry(tries=10, wait=0.2, where="runner-final")
        except Exception: score_obj = None
        wall = time.time() - t0
...
# ==================== 示例 main：把你的 PPOAgent 接进来即可 ====================
def main():
    from exp_3_4_ppo_finetune_seq import PPOAgent  # 确保同目录或已在 PYTHONPATH
    ...
    # === 按你的模型目录加载 PPO（比如 ./bc_out_seq 或更深的时间戳子目录）===
    PPO_BC_DIR = "./bc_out_seq"  # 改成你的目录
    ppo_agent = PPOAgent(bc_dir=PPO_BC_DIR, device="cpu")  # 如需 GPU 改成 "cuda:0"
    ...
    for ep in range(1, EPISODES + 1):
        ...
        success, score = run_one_episode(
            ...
            ppo_agent=ppo_agent,
            max_wall_time_sec=MAX_WALL_TIME_PER_EP,
            min_wall_time_sec=MIN_WALL_TIME_PER_EP,
            explore=True  # 采样期 True；评估可改 False
        )
