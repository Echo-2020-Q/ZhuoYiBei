...
# vel_0.rate = 200
# vel_0.direct = 90
# 注意：本环境速度/角度两个字段与我们语义互换：
#   vcmd.rate   <- 水平速度标量
#   vcmd.direct <- 航向角(度, 0=北, 顺时针)
# 这里先保留默认高度分量 vz，具体速度/航向后面按计划单独下发
vel_0.rate = 0
vel_0.direct = 90
...
class RedForceController:
    def __init__(self, client, red_ids, out_csv_path):
        ...
        self.MISSILE_BEARING_THRESH_DEG = 15.0   # 视线角阈值：导弹航向与“导弹->红机连线”夹角小于该值视为“冲向我”
        ...
        # --- 新增：速度计划状态 ---
        self._init_speed_done = False  # 是否完成开局定速
        self._boost_done = {rid: False for rid in self.red_ids}  # 提速是否已完成（逐机）
    ...
    def step(self):
        ...
        sim_sec = int(sim_t)
        # 到点把非 10085 的三架提到全速
        if self._init_speed_done:
            try:
                if sim_t >= BOOST_AFTER_SEC:
                    for rid in sorted(self.red_ids):
                        if int(rid) == 10085:
                            # 10085 已是全速，无需再提
                            self._boost_done[rid] = True
                            continue
                        if not self._boost_done.get(rid, False):
                            vcmd = rflysim.Vel()
                            vcmd.rate = float(FULL_SPEED)  # 提速到全速
                            vcmd.direct = EAST_HEADING_DEG  # 保持向东
                            # vz 保持当前垂直速度（或改成固定 vel_0.vz 也行）
                            try:
