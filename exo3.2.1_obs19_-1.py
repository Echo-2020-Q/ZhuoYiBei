...
class RLRecorder:
    ...
    def pack_single_obs19(
        ...
        # --- 原来是：nz(nearest_msl_dir) % 360.0 ... ---
        #     为了让 -1 不被取模成 359，这里特殊对待 -1 这个哨兵值
        if nearest_msl_dir is None:
            msl_dir_val = 0.0
        else:
            try:
                _dval = float(nearest_msl_dir)
                if _dval == -1.0:
                    msl_dir_val = -1.0   # ★ 保留 -1，不取模
                else:
                    msl_dir_val = _dval % 360.0
            except Exception:
                msl_dir_val = 0.0
        ...
        obs19 = [
            ...
            *[nz(d) for d in blue4_dists],
            ...
            nz(nearest_msl_dist),
            msl_dir_val,  # ★ 这里用上面的 msl_dir_val
            ...
            for rid in sorted(self.red_ids):
                ...
                if my_lon is not None and my_lat is not None and missiles_pos:
                    ...
                    if best_mid is not None:
                        ...
                        nearest_msl_dir = -1.0  # ★ 观测到导弹 -> 固定写 -1
