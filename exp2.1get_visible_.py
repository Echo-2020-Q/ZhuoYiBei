...
class RedForceController:
    def __init__(self, client, red_ids):
        ...
        for rid in red_ids:
            try:
                #打开雷达
                uid = self.client.enable_radar(rid, 1)
                #设置初始速度
                velocity_uid = self.client.set_vehicle_vel(rid, vel_0)
                #检查命令
                time.sleep(0.1)
                print(f"[Red] Radar ON for {rid}, uid={uid}", flush=True)
                print(client.get_command_status(uid))           #print(f"[Red] Radar ON for {rid}, uuid={client.get_command_status(uid)}")
                velocity_status=client.get_command_status(velocity_uid)
                time.sleep(0.1)
                print(velocity_status)  #print(f"[Red] velocity set for {rid}, uuid={client.get_command_status(velocity_uid)}")
    ...
    try:
        while True:
            ...
            if print(client.get_score(), flush=True)== None:
                ...
                print("分数：",client.get_score(), flush=True)

            print("态势信息：",client.get_situ_info())
