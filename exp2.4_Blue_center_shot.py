...
class RedForceController:
    ...
    def _update_destroyed_from_situ(self, force=False):
        ...
        if not force:
            ...
            if self._situ_counter % 5 != 0:  # 每5次step才真正调用
                return
        ...
        # 诊断：若解析结果为空，打一条一次性提示（避免刷屏）
        if not situ and not hasattr(self, "_printed_situ_warn"):
            print("[WARN] situ parsing returned empty. Raw=", type(situ_raw), situ_raw, flush=True)
        ...
        for vid, info in situ.items():
            ...
            if isinstance(dmg, str) and dmg.upper() == DESTROYED_FLAG:
